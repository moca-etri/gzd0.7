		...
		if (remaining == 0) {
			...
			if (bbio->bio) {
				if (bbio->status == 0) {	/* no error */
					blk_end_request_all(bbio->req, 0);
				} else {	/* -EIO */
...
retry:
		...
		if (ret == -EBUSY) {
			...
			pr_debug
			    ("%s: sleeping due to bio_request EBUSY, tag %d\n",
			     __func__, tag);
			err =
			    wait_event_interruptible_timeout(dev->card_data->
							     block_io_queue,
							     dev->card_data->
							     block_io_ready,
							     GZD_TIMEOUT);
...
static int bdev_make_request_queue(struct request *req)
{
	...
	struct gzd_bdev *dev = req->q->queuedata;
	...
	if (rq_end_sector(req) > get_capacity(req->bio->bi_bdev->bd_disk)) {
	...
	rq_for_each_segment(bvec, req, iter) {
		...
		bbio = bdev_get_bbio_req(iter.bio, req, blk_rq_cur_bytes(req));
...
io_error:
	pr_debug("bvec_ret(in rq loop) error !!!\n");
	return -EIO;
...
{
	struct request *req;
	struct gzd_bdev *dev = queue->queuedata;
	...
	while ((req = blk_fetch_request(queue)) != NULL) {
		...
		case GZD_IRQ_NONE:
			...
			if (blk_rq_cur_bytes(req) > 4096) {
				pr_debug("total rq size %d bytes",
...
int gzdb_prep(struct request_queue *queue, struct request *req)
{
	int ret;

	ret = blk_queue_start_tag(queue, req);
...
static ssize_t gzd_cdev_write(struct file *filp, const char __user * buf,
			      size_t count, loff_t * fpos)
{
	struct gzd_cdev *dev = (struct gzd_cdev *)filp->private_data;
	uint64_t *media, val;
