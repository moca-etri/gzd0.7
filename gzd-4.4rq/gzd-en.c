...
MODULE_PARM_DESC(tx_wake_thresh, "minimum free tx FIFO slots before waking");

/* Revisit: temporary debug */
static uint tx_intr_udelay = 0;
module_param(tx_intr_udelay, int, S_IRUGO);
MODULE_PARM_DESC(tx_intr_udelay, "number of usecs to delay in tx_comp_irq_handler");
...
static void gzd_en_free_req_id(struct gzd_en_card_data *card, int req_index);
...
static irqreturn_t gzd_en_tx_comp_irq_handler(int irq, void *data)
{
	...
	do {
		...
		if (ret < 0) {
			dev_err(dev, "%s: msg_response returned error: %d for req_id %u\n",
				__func__, ret, req_id);
		...
		if (ret == 1 || ret == -EIO) {  /* req_id is valid */
			...
			dev_dbg(dev, "%s: req_id=%u, intr_time=%llu, resp_time=%llu, cpu=%d, free_reqs=%u\n",
				__func__, req_id, start, resp_cycles, cpu_id, free_reqs);
			msg_work = kmalloc(sizeof(*msg_work), GFP_ATOMIC);
			if (!msg_work) {
				netdev_err(card->netdevs[GZD_NETDEV_INDEX], "msg_work kmalloc failed\n");
				return IRQ_NONE;
			}
			msg_work->card = card;
	...
	if (!handled)
		dev_dbg(dev, "%s: spurious, intr_time=%llu, resp_time=%llu, cpu=%d, free_reqs=%u\n",
...
static void gzd_en_rx_work_handler(struct work_struct *w)
{
	...
	do {
		...
		do {
			...
			packet_len_after_wrap = packet_len -
				packet_len_before_wrap;
			/* alloc & memcpy msg */
			skb = __netdev_alloc_skb_ip_align(
				card->netdevs[GZD_NETDEV_INDEX], packet_len,
				GFP_KERNEL);
			if (!skb) {
...
static inline void gzd_en_eth_hw_addr(struct net_device *dev,
				      uint index, uint cid)
{
...
static int gzd_en_probe(void *arg)
{
	...
	dev_info(dev, "gzd_en: hw ptr: %p, card_data: %p\n", card_data->hw, card_data);
	dev_info(dev, "gzd_en: gzd_core_info: card_id=%u, num_cards=%u, num_media=%u "
	...
	for (i = 0; i < info->num_cards; i++)
		dev_info(dev, "gzd_en: gzd_core_info:   card_ids[%d]=%u, scid=0x%x, dcid=0x%x\n",
			i, info->card_ids[i], info->msg_info[i].scid,
			info->msg_info[i].dcid);

	/* Register interrupts  */
	ret = gzd_en_driver.request_irq((void *)card_data->hw, MsgSendInt, gzd_en_tx_comp_irq_handler, 0, "gzd-en-msg-send", (void *)card_data);
	if (ret) {
		dev_err(dev, "Error requesting MsgSendInt\n");
		goto out;
	}
	ret = gzd_en_driver.request_irq((void *)card_data->hw, MsgRecvInt, gzd_en_rx_irq_handler, 0, "gzd-en-msg-recv", (void *)card_data);
	if (ret) {
	...
	for (i = 0; i < GZD_NUM_NETDEVS; i++) {
		struct gzd_en_netdev_priv *priv;
		card_data->netdevs[i] = alloc_netdev(sizeof(struct gzd_en_netdev_priv), "gzd%d", NET_NAME_UNKNOWN,
			gzd_net_init);
...
static int gzd_en_alloc_req_id(struct gzd_en_card_data *card)
{
	...
	if (card->tx_pending[req_index].dma_addr) {
		netdev_err(card->netdevs[GZD_NETDEV_INDEX],
			...
			__func__, req_id, &card->tx_pending[req_index].dma_addr);
...
int gzd_en_tx(struct sk_buff *skb, struct net_device *netdev)
{
	...
	if (req_id < 0) {
		netdev_err(card->netdevs[GZD_NETDEV_INDEX], "%s() no free req_id\n", __func__);
		priv->stats.tx_errors++;
		return -ENOMEM;
	}
	req_index = req_id - MIN_REQ_ID;
	...
	if (dma_mapping_error(&card->core_info->pdev->dev, dma_addr)) {
		...
		netdev_err(card->netdevs[GZD_NETDEV_INDEX], "gzd_en_tx dma mapping failed.\n");
		priv->stats.tx_errors++;
		return -ENOMEM;
	}
	dma_sync_single_for_device(dev, dma_addr,
				  len, DMA_TO_DEVICE);
#endif
	/* fill in all tx_pending info before calling msg_request as
	   that could immediately generate a tx interrupt that uses these */
...
static void gzd_en_get_drvinfo(struct net_device *dev,
...
static void gzd_en_get_ethtool_stats(struct net_device *dev,
...
static int gzd_en_get_ts_info(struct net_device *dev,
