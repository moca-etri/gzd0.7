...
MODULE_PARM_DESC(msg_intr_udelay, "number of usecs to delay in msg_recv_interrupt");

static char *recv_buf_size = "2M";
module_param(recv_buf_size, charp, S_IRUGO);
MODULE_PARM_DESC(recv_buf_size, "size of msg recv buffer in bytes, with optional [KMG]");
...
static struct pci_device_id gzd_core_id_table[] = {
	{ PCI_VDEVICE(HP_3PAR, 0x020B), 0 }, /* AlphaData */
...
enum {
	...
	EccStatus       = 0x200000,   /* DDR4 ECC Status */
};
...
static const char *interface_state_name[] = { "I-Down", "I-CFG", "I-Up", "I-LP",
					      "Rv4", "Rv5", "Rv6", "Rv7"};

/* ============================================================
 *                    BLUE BLOCK INITIALIZATION
 * ============================================================ */
...
static uint64_t bitfield_insert(uint64_t cur, uint pos, uint len, uint64_t new)
{
	uint64_t mask;
...
static int gzd_core_reg_block_init(struct gzd_core_hw *hw,
				const struct gzd_core_block_init * const block)
{
	...
	for (reg = block->table; reg->start_offset != 0xfff; reg++) {
		value = reg->value;
		if (reg->n.len)
			value = bitfield_insert(value, reg->n.pos, reg->n.len,
						card_id);
		if (reg->m.len)
			value = bitfield_insert(value, reg->m.pos, reg->m.len,
						block->m);
		if (reg->z.len)
...
static int gzd_core_reg_block_compare(struct gzd_core_hw *hw,
				const struct gzd_core_block_init * const block)
{
...
/* ============================================================
 *                   SWITCH CORE ROUTING SYSFS FUNCTIONS
 * ============================================================ */
static ssize_t gzd_core_show_sci_index(struct device *ddev,
...
static ssize_t gzd_core_store_sci_index(struct device *ddev,
					struct device_attribute *attr,
...
static ssize_t gzd_core_show_sci_cid(struct device *ddev,
				     struct device_attribute *attr,
				     char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_store_sci_cid(struct device *ddev,
				      struct device_attribute *attr,
				      const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_show_sci_route(struct device *ddev,
				       struct device_attribute *attr,
				       char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_store_sci_route(struct device *ddev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
/* ============================================================
 *                   MEMORY MAP TESTING SYSFS FUNCTIONS
 * ============================================================ */
static ssize_t gzd_core_show_memory_map_offset(struct device *ddev,
					struct device_attribute *attr,
					char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_store_memory_map_offset(struct device *ddev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_show_memory_map_size(struct device *ddev,
					struct device_attribute *attr,
					char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_store_memory_map_size(struct device *ddev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
	...
	/* re-adjust the memory map offset to avoid non-aligned
	   accesses when setting the offset first, then the size second */
	hw->memory_map_offset = rounddown(hw->memory_map_offset, size);
	return count;
}

static ssize_t gzd_core_show_memory_map_test(struct device *ddev,
					struct device_attribute *attr,
					char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
	...
	dev_info(dev,
		...
		(unsigned long) (hw->media_base_addr + hw->memory_map_offset), hw->memory_map_size);
...
static ssize_t gzd_core_store_memory_map_test(struct device *ddev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
	...
	/* Take the scanned in data, and store it to the mempory map address space */
	if (hw->memory_map_size == 8) {
...
/* ============================================================
 *                    BLOCK I/O TESTING SYSFS FUNCTIONS
 * ============================================================ */
static ssize_t gzd_core_show_block_io_offset(struct device *ddev,
					     struct device_attribute *attr,
					     char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_store_block_io_offset(struct device *ddev,
					      struct device_attribute *attr,
					      const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_show_block_io_size(struct device *ddev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_store_block_io_size(struct device *ddev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_show_block_io_cardid(struct device *ddev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_store_block_io_cardid(struct device *ddev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_show_block_io_test(struct device *ddev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
	...
	cpu_addr = dma_alloc_coherent(&pdev->dev, count, &dma_addr, GFP_KERNEL);
	if (!cpu_addr)
		return -ENOMEM;
...
static ssize_t gzd_core_store_block_io_test(struct device *ddev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
/* ============================================================
 *                    MESSAGING TESTING SYSFS FUNCTIONS
 * ============================================================ */
...
size_t __gzd_core_msg_update_cons_ptr(struct gzd_core_hw *hw,
...
static ssize_t gzd_core_show_msg_offset(struct device *ddev,
					struct device_attribute *attr,
					char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_store_msg_offset(struct device *ddev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_show_msg_cardid(struct device *ddev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_store_msg_cardid(struct device *ddev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_show_msg_cons_ptr(struct device *ddev,
					  struct device_attribute *attr,
					  char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_show_msg_prod_ptr(struct device *ddev,
					  struct device_attribute *attr,
					  char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_show_msg_test(struct device *ddev,
				      struct device_attribute *attr,
				      char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ssize_t gzd_core_store_msg_test(struct device *ddev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
	...
	/* unknown if sysfs guarantees buf is DMAable, so alloc & copy */
	cpu_addr = dma_alloc_coherent(&pdev->dev, count+offset, &dma_addr, GFP_KERNEL);
	if (!cpu_addr)
		return -ENOMEM;
	clear_bit(req_index, hw->msg_done);
	memcpy(cpu_addr+offset, buf, count);
	ret = __gzd_core_msg_request(hw, dma_addr+offset, count, req_id, dcid);
	if (ret <  0) {
		dma_free_coherent(&pdev->dev, count, cpu_addr, dma_addr);
...
static ssize_t gzd_core_show_block_status(struct device *ddev,
					  struct device_attribute *attr,
					  char *buf)
{
...
static ssize_t gzd_core_show_msg_status(struct device *ddev,
					struct device_attribute *attr,
					char *buf)
{
...
static ssize_t gzd_core_show_addr_trans_status(struct device *ddev,
					struct device_attribute *attr,
					char *buf)
{
...
static ssize_t gzd_core_show_addr_trans_error(struct device *ddev,
					struct device_attribute *attr,
					char *buf)
{
...
static ssize_t gzd_core_show_seq_status(struct device *ddev,
					struct device_attribute *attr,
					char *buf)
{
...
static ssize_t gzd_core_store_status(struct device *ddev,
				     struct device_attribute *attr,
				     const char *buf, size_t count,
				     const uint reg)
{
...
static ssize_t gzd_core_store_block_status(struct device *ddev,
					   struct device_attribute *attr,
					   const char *buf, size_t count)
{
...
static ssize_t gzd_core_store_msg_status(struct device *ddev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
...
static ssize_t gzd_core_store_addr_trans_status(struct device *ddev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
...
static ssize_t gzd_core_store_addr_trans_error(struct device *ddev,
					       struct device_attribute *attr,
					       const char *buf, size_t count)
{
...
static ssize_t gzd_core_store_seq_status(struct device *ddev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
 ...
 *                    CORE SYSF FUNCTIONS
 * ============================================================ */
static ssize_t gzd_core_show_card_id(struct device *ddev,
				     struct device_attribute *attr,
				     char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
/* ============================================================
 *                    ADDRESS TRANSLATION INITIALIZATION
 * ============================================================ */
static void gzd_core_read_addr_trans_entry(struct gzd_core_hw *hw, uint index,
...
static ssize_t gzd_core_show_addr_trans(struct device *ddev,
					struct device_attribute *attr,
					char *buf)
{
	struct pci_dev *pdev = to_pci_dev(ddev);
...
static ulong gzd_core_write_addr_trans_entry(struct gzd_core_hw *hw, uint index)
{
	struct device *dev = &hw->pdev->dev;
	...
	if (size > 0) {
		start = hw->info.media_info[index].mmio_addr -
			hw->media_mmio_start;
		end = start + size - 1;
...
static void gzd_core_hw_info_init(struct gzd_core_hw *hw)
{
	...
	hw->info.num_media = min(max((uint)media_size_argc / hw->info.genz_subnets, hw->info.num_cards),
				MAX_MEDIA / hw->info.genz_subnets);
	for (i = 0, j = hw->card_index; i < hw->info.num_media;
		...
		hw->info.media_info[i].mmio_addr = hw->media_mmio_start +
			total_size;
		hw->info.media_info[i].base_addr = hw->media_base_addr +
			total_size;
		total_size += size;
	}
...
/* jsma add */
//	hw->info.req_fifo_depth  = 10;
//        pr_debug("HwParams-- hw request_fifo_depth %d\n",hw->info.req_fifo_depth);
/* end */

	hw->info.resp_fifo_depth = (1u << bitfield_extract(params, 8, 8)) - 1u;

/* jsma add */
//        pr_debug("HwParams-- hw response_fifo_depth %d\n",hw->info.resp_fifo_depth);
/* end */
	...
	hw->info.zlink_mask |=
		(hw->info.zlink_mask & ((1u << MAX_MZLINKS) - 1u))
		<< MAX_ZLINKS;
}
...
static void gzd_core_remove_hw(struct gzd_core_hw * const hw)
{
	...
	dma_free_coherent(&hw->pdev->dev, hw->msg_tbl.buf_size + MSG_BUF_OFFSET,
...
static uint gzd_core_interface_enable(struct gzd_core_hw * const hw,
	...
	for (i = 0; i < 100; i++) {
		for (j = 0; j < MAX_AURORA; j++) {
			if (AU_MASK(hw->info.zlink_mask, j))
				status[j] = ioread64(hw->regs_base_addr +
						     au_offset[j]);
		}
	...
	for (i = done = 0; i < 100 && done < num_zlinks; i++) {
		for (j = 0; j < (MAX_ZLINKS+MAX_MZLINKS); j++) {
			if (!ZLINK_MASK(hw->info.zlink_mask, j) || zdone[j])
				continue;
			cur_state = gzd_core_interface_enable(hw,
							      zl_offset[j], 0);
...
// irq status save and entering spinlock 
	spin_lock_irqsave(&hw->lock, flags);

	free_reqs = hw->info.req_fifo_depth - hw->block_num_reqs;
// jsma
//	pr_debug("%s: free request size is : %d\n",__func__, free_reqs);

	free_resps = hw->info.resp_fifo_depth - hw->block_num_resps -
		hw->block_num_reqs;
// jsma
//	pr_debug("%s: free response size is : %d\n",__func__, free_resps);

	/* There is a small window in the HW where an outstanding request
	   is not tracked by either the HW num_reqs or num_resps value.
	   To compensate, we leave one extra slot in the resp FIFO - hence
	   the "> 1" here and the "<= 2" in the EBUSY if. */

	if (!(free_reqs > 0 && free_resps > 1)) {

		/* our driver-maintained counts indicate full or overflow -
		   check the HW before returning an error */
...
//	pr_debug("%s: block is num of request is : %d\n",__func__, num_reqs);

//	pr_debug("%s: block is num of response is : %d\n",__func__, num_resps);
	...
	if (overflow || free_reqs < 0 || free_resps < 0) {
		...
		dev_err(&hw->pdev->dev, "%s overflow=%u, free_reqs=%d, free_resps=%d\n",
			__func__, overflow, free_reqs, free_resps);
		err = -ENOSPC;
		goto unlock;
...
static int gzd_core_block_io_request(void *arg, dma_addr_t host_addr,
				     uint64_t genz_addr, uint len,
				     uint req_id, uint dcid,
				     uint rw /* w=0, r=1 */)
{
...
static int __gzd_core_block_io_response(struct gzd_core_hw *hw,
...
//   jsma
	done          = (uint)bitfield_extract(resp,  8, 1);
	num_resps     = (uint)bitfield_extract(resp, 56, 8);
	num_reqs      = (uint)bitfield_extract(resp, 48, 8);
	req_overflow  = (uint)bitfield_extract(resp, 13, 1);
	resp_overflow = (uint)bitfield_extract(resp, 15, 1);
	...
	*free_reqs = (uint)max(0,
			min((int)(hw->info.req_fifo_depth - num_reqs),
			    (int)(hw->info.resp_fifo_depth - num_resps -
				  num_reqs - 1)));

	if (done) {  /* get req_id, check for NAK, advance response FIFO */
...
/* jsma */
	//	pr_debug("advanced value check %d\n", advance);
// jsma end 
...
//			__func__, num_resps, num_reqs, *req_id, nak);
	...
	if (req_overflow || resp_overflow) { /* clear FIFOs, return error */
		if (req_overflow) {
...
/* Return     Reason         req_id valid?  free_reqs valid?
   -EINVAL    bad hw ptr          N                 N
   -ENOSPC    FIFO overflow       N                 Y
   -EIO       NAK response        Y                 Y
   0          no response         N                 Y
   1          valid response      Y                 Y
*/
static int gzd_core_block_io_response(void *arg, uint *req_id, uint *free_reqs)
{
	struct gzd_core_hw *hw;
/* jsma mod */
...
static int gzd_core_msg_request(void *arg, dma_addr_t host_addr, uint len,
				uint req_id, uint dcid)
{
	struct gzd_core_hw *hw;
...
static int __gzd_core_msg_response(struct gzd_core_hw *hw,
				   uint *req_id, uint *free_reqs)
{
	cycles_t start = get_cycles();
	ulong flags;
	...
	*free_reqs = (uint)max(0,
	...
	dev_dbg(&hw->pdev->dev,
		...
		__func__, start, get_cycles(), *req_id, err ? err : done, *free_reqs, __builtin_return_address(0));
	return err ? err : done;
}
...
/* Return     Reason         req_id valid?  free_reqs valid?
   ...
   1          valid response      Y                 Y
*/
static int gzd_core_msg_response(void *arg, uint *req_id, uint *free_reqs)
{
	struct gzd_core_hw *hw;
...
static int gzd_core_msg_test_recv(struct gzd_core_hw *hw,
				  struct gzd_msg_header *msg_hdr)
{
	struct device *dev = &hw->pdev->dev;
...
static int gzd_core_write_msg_tbl_entry(struct gzd_core_hw *hw, uint index,
...
static int gzd_core_msg_tbl_init(void *arg,
...
/* returns current prod_ptr offset */
/* also returns current prod_ptr and prod_ptr_va in tbl[index] */
uint64_t gzd_core_msg_read_prod_ptr(void *arg, struct gzd_msg_tbl_entry *tbl,
				    uint index)
{
	struct gzd_core_hw *hw;
...
/* returns how far behind prod_ptr the updated cons_ptr is */
/* also returns current prod_ptr in tbl[index].prod_ptr */
size_t gzd_core_msg_update_cons_ptr(void *arg, struct gzd_msg_tbl_entry *tbl,
				uint index, size_t size)
{
	struct gzd_core_hw *hw;
...
/* forward declarations */
static int __gzd_core_request_irq(struct gzd_core_hw *hw, uint irq_index,
...
static int gzd_core_thread(void *data)
{
	...
	while (1) {
		...
		for (j = 0; j < (MAX_ZLINKS+MAX_MZLINKS); j++) {
			...
			cur_state[j] = gzd_core_interface_enable(hw,
							    zl_offset[j], 1);
...
static int gzd_core_probe(struct pci_dev *pdev,
...
//	if (pci_enable_msi_exact(pdev, GZD_CORE_MSI_COUNT)) {
	nvec=pci_alloc_irq_vectors(pdev,1,GZD_CORE_MSI_COUNT, PCI_IRQ_MSI | PCI_IRQ_MSIX);
	if(nvec < 0 ){
	...
	dev_info(dev, "gzd-core version %s, FPGA ID %s rev %llu.%llu (0x%016llx)\n",
		GZD_DRV_VERS,
		gzd_core_sprintf_xilinx_timestamp(timestamp, timestr, 32),
		major, minor, hwid);
	...
	if (gzd_core_blue_block_init(hw) != 0) {
...
// jsma core request
	err = __gzd_core_request_irq(hw, BlockRWInt,
				...
				0, GZD_CORE_IRQ_ID "-block-rw", hw, GzdCoreIrq);
	...
	err = __gzd_core_request_irq(hw, MsgSendInt,
				...
				0, GZD_CORE_IRQ_ID "-msg-send", hw, GzdCoreIrq);
	if (err)
		goto irq_release;
	...
	if (!cpu_addr)
		goto irq_release;
	/* Revisit: for red/green-only bitstreams, assume "loopback"
	   to ourselves; otherwise, assume there's a second card */
	if (red_green)
		hw->msg_tbl.scid =
			hw->info.msg_info[hw->info.card_index].scid;
	else
		hw->msg_tbl.scid =
			hw->info.msg_info[hw->info.card_index+1].scid;
	/* use first 8 bytes for prod_ptr; buf_start is next
	   MSG_BUF_OFFSET aligned location */
	...
	err = __gzd_core_request_irq(hw, MsgRecvInt,
				...
				0, GZD_CORE_IRQ_ID "-msg-recv", hw, GzdCoreIrq);
	if (err)
		goto dma_release;
	err = __gzd_core_request_irq(hw, ByteAddrErrInt,
				gzd_core_error_interrupt,
				0, GZD_CORE_IRQ_ID "-byte-addr-err", hw, GzdCoreIrq);
	if (err)
		goto dma_release;
	err = __gzd_core_request_irq(hw, AddrTransErrInt,
				gzd_core_error_interrupt,
				0, GZD_CORE_IRQ_ID "-addr-trans-err", hw, GzdCoreIrq);
	if (err)
		goto dma_release;
	err = __gzd_core_request_irq(hw, HWApiErrInt,
				gzd_core_error_interrupt,
				0, GZD_CORE_IRQ_ID "-hw-api-err", hw, GzdCoreIrq);
	if (err)
		goto dma_release;
	err = __gzd_core_request_irq(hw, BrSeqErrInt,
				gzd_core_error_interrupt,
				0, GZD_CORE_IRQ_ID "-br-seq-err", hw, GzdCoreIrq);
	if (err)
		goto dma_release;
...
/* ============================================================
 *                    THE DEPENDENT DRIVER INTERFACE
 * ============================================================ */
...
static void gzd_core_enable_intr(struct gzd_core_hw *hw, uint irq_index,
				 uint state)
{
	ulong flags;
	uint64_t val;
...
static int gzd_core_request_irq(void *arg, uint irq_index,
				irq_handler_t handler, ulong irqflags,
				const char *devname, void *dev_id)
{
	struct gzd_core_hw *hw;
			       ...
			       void *dev_id, uint irq_type)
{
	...
	prev_irqflags = hw->irq_info[irq_index].irq_save[prev_irq_type].irqflags;
	...
	ret = request_irq(hw->base_irq + irq_index, prev_handler, prev_irqflags,
...
//	gzd_core_wq = create_workqueue("gzd_core_wq");
